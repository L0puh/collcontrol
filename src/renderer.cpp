...
void check_collisions_FIXME(std::vector<Object> *objects){
   for(int i = 0; i < objects->size(); i++){
      for(int j = i+1; j < objects->size(); j++){
         if (objects->at(i).shape->type == shape_type::rectangle &&
                  objects->at(j).shape->type == shape_type::rectangle){
...
void Renderer::drag_and_drop(std::vector<Object> *objects){
   for(int i = objects->size()-1; i >= 0; i--){
      Object obj = objects->at(i);
      glm::vec2 p = camera->get_mouse_pos();

      obj.draw(camera->get_projection(), camera->get_view());
      bool is_inside = collision::point_is_inside(camera->get_mouse_pos(), obj);
      ...
      if (state.mouse_clicked && !state.imgui_focused && current_id == obj.id){
         obj.set_pos(glm::vec3((p.x - last_pos.x), (p.y - last_pos.y), 0.0f));
      } 
      if (!state.mouse_clicked && current_id != -1) current_id = -1;
      objects->at(i) = obj;
   }
}
void Renderer::render(std::vector<Object> *objects) {
   drag_and_drop(objects);
